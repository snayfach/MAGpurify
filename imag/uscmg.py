...
def extract_homologs(out_dir):
	# create outdir
	seqs_dir = "%s/seqs" % out_dir
	if not os.path.isdir(seqs_dir):
		os.makedirs(seqs_dir)
	# fetch best hits from hmmsearch
	gene_to_aln = fetch_hmm_best_hits("%s/phyeco.hmmsearch" % out_dir)
	# open output files
	outfiles = {}
	...
	# write seqs
	for ext in ['ffn', 'faa']:
		in_path = "%s/genes.%s" % (out_dir, ext)
		for rec in parse_fasta(in_path):
			if rec.id in gene_to_aln:
				marker_id = gene_to_aln[rec.id]['qacc']
				seq = str(rec.seq).upper().rstrip('*')
				outfiles[marker_id][ext].write('>'+rec.id+'\n'+seq+'\n')
	# close files
	for marker_id in outfiles:
		outfiles[marker_id]['ffn'].close()
		outfiles[marker_id]['faa'].close()

def align_homologs(db_dir, out_dir, seq_type, threads):
	aln_dir = "%s/alns" % out_dir
	if not os.path.exists(aln_dir):
		os.makedirs(aln_dir)
	seq_files = os.listdir("%s/seqs" % out_dir)
	for file_index, seq_file in enumerate(seq_files):
		...
		if ext == 'faa':
			...
			query_path = '%s/seqs/%s.%s' % (out_dir, marker_id, ext)
			out_path = '%s/alns/%s.%s.m8' % (out_dir, marker_id, ext)
			run_blastp(db_path, query_path, out_path, threads)
		else:
			program = 'blastn'
			db_path = '%s/uscmg/%s/%s' % (db_dir, program, marker_id)
			query_path = '%s/seqs/%s.%s' % (out_dir, marker_id, ext)
...
def flag_contigs(db_dir, out_dir, args):
	...
	for aln_file in os.listdir('%s/alns' % out_dir):
	...
	for marker_id in marker_ids:
		if 'B' in marker_id:
			...
			sys.exit('Marker id not recognized as bacteria or archaea')
	...
	bin = Bin()
	bin.genes = {}
	hmm_path = "%s/phyeco.hmmsearch" % args['out_dir']
	for gene_id, aln in fetch_hmm_best_hits(hmm_path).items():
	...
	for seq_type in seq_types:
		for marker_id in markers:
			aln_path = args['out_dir']+"/alns/"+marker_id+"."+seq_type+".m8"
			for aln in parse_blast(aln_path):
	...
	bin.contigs = {}
	for r in parse_fasta(args['fna_path']):
		bin.contigs[r.id] = Contig()
		bin.contigs[r.id].id = r.id
		bin.contigs[r.id].length = len(r.seq)
	...
	sys.stderr.write("  total marker genes in bin: %s\n" % len(bin.genes))
	sys.stderr.write("  genes supporting bin taxonomy: %s\n" % bin.cons_count)
	sys.stderr.write("  genome length: %s\n" % sum([c.length for c in bin.contigs.values()]))
	sys.stderr.write("  total contigs: %s\n" % len(bin.contigs))
	sys.stderr.write("  # contigs with marker gene: %s\n" % sum([1 for c in bin.contigs.values() if c.flagged is not None]))
	sys.stderr.write("  length of contigs with marker gene: %s\n" % sum([c.length for c in bin.contigs.values() if c.flagged is not None]))
	sys.stderr.write("  # of flagged contigs: %s\n" % sum([1 for c in bin.contigs.values() if c.flagged]))
	sys.stderr.write("  length of flagged contigs: %s\n" % sum([c.length for c in bin.contigs.values() if c.flagged]))
	flagged_contigs = []
	for contig in bin.contigs.values():
		if contig.flagged:
...
def main(args):
	...
	sys.stderr.write("running prodigal\n")
	run_prodigal(args['fna_path'], args['out_dir'])

	sys.stderr.write("running hmmsearch\n")
	run_hmmsearch(args['db'], args['out_dir'], args['threads'])

	sys.stderr.write("extracting homologs\n")
	extract_homologs(args['out_dir'])

	sys.stderr.write("running blast\n")
	align_homologs(args['db'], args['out_dir'], args['seq_type'], args['threads'])

	sys.stderr.write("flagging contigs\n")
	flagged_contigs = flag_contigs(args['db'], args['out_dir'], args)
...
if __name__ == "__main__":
	...
	check_input(args)
	check_output(args)
	check_dependencies(['prodigal', 'hmmsearch', 'blastp', 'blastn'])
	check_database(args)
