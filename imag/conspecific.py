...
def run_mash(db_dir, fna_path, out_dir, threads=1):
	db_path = '%s/sketch.msh' % db_dir
	out_path = '%s/mash.dist' % out_dir
	command = "mash dist -p %s -d 0.25 %s %s > %s" % (threads, fna_path, db_path, out_path)
	out, err = run_process(command)

	with open(out_dir+'/id_map.tsv', 'w') as f:
		for id, rec in enumerate(parse_mash(out_path)):
			f.write(str(id)+'\t'+rec['target']+'\n')

def find_conspecific(out_dir, max_dist, exclude, max_targets=10):

	targets = []
	for rec in parse_mash('%s/mash.dist' % out_dir):
...
def find_contig_targets():
	...
	target_to_id = {}
	for line in open(args['out_dir']+'/id_map.tsv'):
	...
	for genome_path, mash_dist in genomes:
		...
		blast_out = blastn(args['fna_path'], genome_path, args['out_dir'], target_to_id[target])
...
if __name__ == "__main__":
	...
	run_mash(args['db'], args['fna_path'], args['out_dir'], args['threads'])
	
	genomes = find_conspecific(args['out_dir'], args['mash_dist'], args['exclude'], args['max_genomes'])
