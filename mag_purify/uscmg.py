...
def extract_homologs(tmp_dir):
	...
	gene_to_aln = fetch_hmm_best_hits("%s/phyeco.hmmsearch" % tmp_dir)
	# open output files
	outfiles = {}
...
def main(args):
	...
	tmp_dir = '%s/tmp/uscmg' % args['out_dir']
	if not os.path.exists(tmp_dir):
		os.makedirs(tmp_dir)
	
	sys.stderr.write("running prodigal\n")
	run_prodigal(args['fna_path'], tmp_dir)

	sys.stderr.write("running hmmsearch\n")
	run_hmmsearch(args['db'], tmp_dir, args['threads'])

	sys.stderr.write("extracting homologs\n")
	extract_homologs(tmp_dir)

	sys.stderr.write("running blast\n")
	align_homologs(args['db'], tmp_dir, args['seq_type'], args['threads'])

	sys.stderr.write("flagging contigs\n")
	flagged_contigs = flag_contigs(args['db'], tmp_dir, args)
